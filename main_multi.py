...
for filename in os.listdir(train_data_path + "/dog"):
    # read the image
    # print("/content/drive/MyDrive/data/train/dog/" + filename)
    
    
    image = cv2.imread(train_data_path + "/dog/" + filename, 1)
    if image is None:
      continue
...
#read the test data from test_data_path
...
import csv
with open(output_path + "/test_32a.csv", "w") as f:
    
    writer = csv.writer(f)
    for i in range(len(test_predictions)):
        
        #write the image name and the prediction label
        writer.writerow([test_data_names[i], test_predictions[i]])
...
# # Visualize the best-pruned tree as image
# import graphviz
# from sklearn.tree import export_graphviz

# dot_data = export_graphviz(clf, out_file="tree.dot", max_depth=10, feature_names=None, class_names=None, label='all', filled=True, leaves_parallel=False, impurity=True, node_ids=False, proportion=False, rotate=False, rounded=False, special_characters=False, precision=3)
# # graph = graphviz.Source(dot_data)
# # graph.render("best_pruned_tree")
# # graph


# !dot -Tpng tree.dot -o best_pruned_tree.png
...
# save the predicted labels into a csv file
import csv
with open(output_path + "/test_32b.csv", "w") as f:
...
best_alpha = 0.0016969696969696972
...
# save the predicted labels into a csv file
import csv
with open(output_path + "/test_32c.csv", "w") as f:
...
# save the predicted labels into a csv file
import csv
with open(output_path + "/test_32d.csv", "w") as f:
...
# predict the labels for the test data
test_predictions = xgb_clf.predict(test_data)
...
# save the predicted labels into a csv file
import csv
with open(output_path + "/test_32e.csv", "w") as f:
...
# save the predicted labels into a csv file
import csv
with open(output_path + "/test_32h.csv", "w") as f:
